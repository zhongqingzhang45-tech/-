import { SETUP_COMPONENT_ID_REGEX, getMainModule, hotUpdateSetupComponent, isSubModule, loadSetupComponent, transformPost, transformSetupComponent } from "./core-CO4Kr_Ww.js";
import { REGEX_NODE_MODULES, REGEX_SETUP_SFC, REGEX_SRC_FILE, REGEX_VUE_SUB, createFilter, detectVueVersion } from "@vue-macros/common";
...
const PrePlugin = createUnplugin((userOptions = {}, meta) => {
	...
	return {
		...
		resolveId(id, importer) {
			...
			if (["rollup", "vite"].includes(meta.framework) && importer && isSubModule(importer)) {
				...
				return this.resolve(id, mainModule, { skipSelf: true });
			}
		},
		loadInclude(id) {
		...
		vite: {
			...
			handleHotUpdate: (ctx) => {
				if (filter(ctx.file)) return hotUpdateSetupComponent(ctx, setupComponentContext);
			}
		}
	};
});
