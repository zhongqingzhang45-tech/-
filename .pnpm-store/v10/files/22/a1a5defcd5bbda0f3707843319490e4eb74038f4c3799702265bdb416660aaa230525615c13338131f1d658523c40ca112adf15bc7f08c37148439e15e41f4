...
function transformShortVmodel({ prefix = "$" } = {}) {
	return (node, context) => {
...
function processDirective(node) {
	for (const [i, prop] of node.props.entries()) {
		if (prop.type !== 7 || prop.arg?.type !== 4 || !prop.arg.content.startsWith(":")) continue;
		prop.arg.loc.start.offset += 1;
		const argName = prop.arg.content.slice(1);
		node.props[i] = {
			...prop,
			name: "model",
			arg: argName.length > 0 ? {
				...prop.arg,
				content: prop.arg.content.slice(1)
			} : void 0
		};
	}
}
function processAttribute(prefix, node, context) {
	for (const [i, prop] of node.props.entries()) {
		if (prop.type !== 6 || !prop.name.startsWith(prefix) || !prop.value) continue;
		...
		const simpleExpression = createSimpleExpression(prop.value.content, false, expLoc, 0);
