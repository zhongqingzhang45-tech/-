import { HELPER_PREFIX, MagicStringAST, babelParse, generateTransform, getLang, importHelperFn, isCallOf, walkAST } from "@vue-macros/common";
import { createTransformContext, parse, traverseNode } from "@vue/compiler-dom";

//#region src/core/constants.ts
const QUERY_NAMED_TEMPLATE = "?vue&type=named-template";
const QUERY_TEMPLATE = "type=template&namedTemplate";
const QUERY_TEMPLATE_MAIN = `${QUERY_TEMPLATE}&mainTemplate`;
const MAIN_TEMPLATE = Symbol();
...
function transformTemplateIs(s) {
	return (node) => {
		if (node.type !== 1 || node.tag !== "template") return;
		const propIs = node.props.find((prop) => prop.type === 6 && prop.name === "is");
		if (!propIs?.value) return;
		const refName = propIs.value.content;
		s.overwrite(node.loc.start.offset, node.loc.end.offset, `<component is="named-template-${refName}" />`);
	};
}
function preTransform(code, id, templateContent) {
	const root = parse(code);
	const templates = root.children.filter((node) => node.type === 1 && node.tag === "template");
	if (templates.length <= 1) return;
	const s = new MagicStringAST(code);
	for (const node of templates) {
		const propName = node.props.find((prop) => prop.type === 6 && prop.name === "name");
		if (!propName) {
...
function postTransform(code, id, customBlocks) {
	...
	const s = new MagicStringAST(code);
	const subTemplates = [];
	for (const node of program.body) if (node.type === "ExportNamedDeclaration" && node.declaration?.type === "FunctionDeclaration" && node.declaration.id?.name === "render") {
		...
		if (params.length > 0) {
			...
			s.appendLeft(node.declaration.body.start + 1, `\n let [${paramsText}] = args`);
		}
	}
	walkAST(program, { enter(node) {
		if (isCallOf(node, ["_createVNode", "_createBlock"]) && isCallOf(node.arguments[0], "_resolveDynamicComponent") && node.arguments[0].arguments[0].type === "StringLiteral" && node.arguments[0].arguments[0].value.startsWith("named-template-")) subTemplates.push({
			vnode: node,
			component: node.arguments[0],
			name: node.arguments[0].arguments[0].value.replace("named-template-", ""),
			fnName: node.callee.name
		});
	} });
	if (subTemplates.length === 0) return;
	for (const { vnode, component, name, fnName } of subTemplates) {
		if (!customBlocks[filename]?.[name]) throw new SyntaxError(`Unknown named template: ${name}`);
		const render = `${HELPER_PREFIX}block_${escapeTemplateName(name)}.render(...args)`;
		if (fnName === "_createVNode") s.overwriteNode(vnode, render);
	...
	for (const [name, source] of Object.entries(customBlocks[filename])) s.prepend(`import ${HELPER_PREFIX}block_${escapeTemplateName(name)} from ${JSON.stringify(source)};\n`);
	return generateTransform(s, id);
}
function postTransformMainEntry(program, id, customBlocks) {
	for (const node of program.body) if (node.type === "ImportDeclaration" && node.source.value.includes(QUERY_NAMED_TEMPLATE)) {
...
//#endregion
export { MAIN_TEMPLATE, QUERY_NAMED_TEMPLATE, QUERY_TEMPLATE, QUERY_TEMPLATE_MAIN, getChildrenLocation, parseVueRequest, postTransform, postTransformMainEntry, preTransform, preTransformMainTemplate, transformTemplateIs };