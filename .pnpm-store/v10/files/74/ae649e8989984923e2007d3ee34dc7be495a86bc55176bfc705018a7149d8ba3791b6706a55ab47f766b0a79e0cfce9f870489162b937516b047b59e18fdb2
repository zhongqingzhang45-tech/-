...
export interface SortingNode<Node extends TSESTree.Node = TSESTree.Node> {
  ...
  /**
   * Indicates whether a safety semicolon should be added when the node is
   * moved inline.
   *
   * Used to prevent Automatic Semicolon Insertion (ASI) issues when
   * reordering statements. For example, when moving a statement that starts
   * with `[` or `(` after another statement without a semicolon, a safety
   * semicolon is added to prevent the two statements from being incorrectly
   * interpreted as one.
   */
   ...
   * belongs to.
   *
   * Partitions divide code into separate blocks that are sorted
   * independently. Nodes with different partition IDs are never moved across
   * partition boundaries, preserving logical code organization. Common
   * partition boundaries include:
   *
  ...
  partitionId: number
  /**
   * The group identifier this node belongs to within the sorting
   * configuration.
   *
   ...
   * then sorted within each group according to the sorting algorithm. The
   * group can be a predefined group, custom group, or 'unknown' for
   ...
   *
   * The sorting algorithm uses this value for alphabetical, natural, or
   * custom sorting comparisons.
   */
  name: string
  /**
