import { o as computed } from "./runtime-core.esm-bundler-DrsXb1EG.js";
import { t as usePayloadStore, y as defineStore } from "./payload-DkARwyuW.js";
import { t as useOptionsStore } from "./options-BTLP_X4i.js";
...
//#endregion
//#region src/client/stores/search.ts
var useSearchResults = defineStore("search", () => {
	...
	const results = computed(() => {
		...
		else return new Fuse(modules, {
			shouldSort: true,
			keys: ["id", "plugins"]
		}).search(state.search.text).map((i) => i.item);
	});
	return {
		results,
		resultsSorted: computed(() => {
