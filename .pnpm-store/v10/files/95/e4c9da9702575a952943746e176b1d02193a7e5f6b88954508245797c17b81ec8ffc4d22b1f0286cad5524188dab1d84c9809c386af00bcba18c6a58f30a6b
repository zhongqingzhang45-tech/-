...
import { mergeConfig as mergeViteConfig, resolveConfig as resolveViteConfig, } from 'vite';
...
export async function loadConfigFile(configFile) {
    try {
        ...
        const result = await jiti.import(configFile, { default: true });
        if (!result) {
...
async function processDefaultConfig(defaultConfig, preUserConfig, mode, _cwd) {
    // Apply plugins
