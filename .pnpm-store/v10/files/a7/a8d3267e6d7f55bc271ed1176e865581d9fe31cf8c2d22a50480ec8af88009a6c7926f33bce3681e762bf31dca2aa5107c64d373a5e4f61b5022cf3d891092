...
function transformShortBind(options = {}) {
	...
	return (node, context) => {
		...
		for (const prop of node.props) if (reg.test(prop.loc.source) && (prop.type === 6 ? !prop.value : prop.type === 7 ? !prop.exp : false)) {
			...
			if (prop.type === 6) {
				...
				const simpleExpression = createSimpleExpression(valueName, false, {
					start: {
				...
				if (prop.arg?.type === 4) prop.arg.loc.start.offset = Number.POSITIVE_INFINITY;
