import { nurseryRules, rules_by_category_exports } from "../generated/rules-by-category.mjs";
import { aliasPluginNames, reactHookRulesInsideReactScope, typescriptRulesExtendEslintRules } from "../constants.mjs";
import { isObject } from "./utilities.mjs";
//#region src/build-from-oxlint-config/rules.ts
var allRules = Object.values(rules_by_category_exports).flatMap((rulesObject) => Object.keys(rulesObject));
var typeAwareRules = new Set(Object.entries(rules_by_category_exports).filter(([key]) => key.endsWith("TypeAwareRules")).flatMap(([, rulesObject]) => Object.keys(rulesObject)));
...
//#endregion
export { handleRulesScope, readRulesFromConfig };
