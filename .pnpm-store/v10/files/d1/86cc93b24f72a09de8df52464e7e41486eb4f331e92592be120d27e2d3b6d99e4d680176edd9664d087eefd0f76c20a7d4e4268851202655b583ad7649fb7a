...
export const allResolutionOptions = ["node10", "node16", "bundler"];
export const allResolutionKinds = ["node10", "node16-cjs", "node16-esm", "bundler"];
export function getResolutionOption(resolutionKind) {
...
export function getResolutionKinds(resolutionOption) {
...
export function resolvedThroughFallback(traces) {
    let i = 0;
    ...
    function conditionalExportsResolvedThroughFallback() {
        i++;
...
export function visitResolutions(entrypoints, visitor) {
...
export function groupProblemsByKind(problems) {
    var _a;
    var _b;
    const result = {};
    for (const problem of problems) {
        ((_a = result[_b = problem.kind]) !== null && _a !== void 0 ? _a : (result[_b] = [])).push(problem);
    }
    return result;
}
