...
import { matchesTsconfigPaths } from './matches-tsconfig-paths.js'
...
var bunModules = /* @__PURE__ */ new Set([
  'detect-libc',
  'bun:sqlite',
  'bun:test',
  'bun:wrap',
...
var nodeBuiltinModules = new Set(builtinModules)
var builtinPrefixOnlyModules = /* @__PURE__ */ new Set([
  'node:sqlite',
  'node:test',
  'node:sea',
])
...
function getInternalOrExternalGroup({ tsConfigOutput, filename, name }) {
  ...
  if (typescriptImport.isExternalModuleNameRelative(name)) {
    return null
  }
  if (!tsConfigOutput) {
    return 'external'
