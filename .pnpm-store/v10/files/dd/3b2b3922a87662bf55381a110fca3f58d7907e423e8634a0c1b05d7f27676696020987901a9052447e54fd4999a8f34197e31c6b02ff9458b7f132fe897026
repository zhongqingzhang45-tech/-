...
const SEP = `(?:\\s*,\\s*|\\s+)`;
const ALPHA_SEP = `\\s*[,/]\\s*`;
const RGB = new RegExp(`^rgba?\\(\\s*(${VALUE})${SEP}(${VALUE})${SEP}(${VALUE})(?:${ALPHA_SEP}(${VALUE}))?\\s*\\)$`);
const HSL = new RegExp(`^hsla?\\(\\s*(${VALUE}(?:deg|rad|grad|turn)?)${SEP}(${VALUE})${SEP}(${VALUE})(?:${ALPHA_SEP}(${VALUE}))?\\s*\\)$`);
export function parseColor(value) {
    ...
    if (hex !== null) {
        return {
            mode: 'rgb',
            color: [Number.parseInt(hex[1], 16), Number.parseInt(hex[2], 16), Number.parseInt(hex[3], 16)].map(v => v.toString()),
