import { DEFINE_SETUP_COMPONENT, HELPER_PREFIX, MagicStringAST, attachScopes, babelParse, generateTransform, getLang, isCallOf, normalizePath, walkAST } from "@vue-macros/common";

//#region src/core/constants.ts
const SETUP_COMPONENT_ID_SUFFIX = "-setup-component-";
const SETUP_COMPONENT_ID_REGEX = /-setup-component-(\d+).vue$/;
const SETUP_COMPONENT_SUB_MODULE = /-setup-component-(\d+).vue.*/;
const SETUP_COMPONENT_TYPE = "SetupFC";

//#endregion
//#region src/core/sub-module.ts
...
function scanSetupComponent(code, id) {
	...
	return {
		components: components.map(({ decl, fn, scopes }) => {
			if (!["FunctionExpression", "ArrowFunctionExpression"].includes(decl.type)) throw new SyntaxError(`${DEFINE_SETUP_COMPONENT}: invalid setup component definition`);
...
function transformSetupComponent(code, _id, ctx) {
	...
	for (const [i, { node, scopes }] of components.entries()) {
		...
		s.overwrite(node.start, node.end, `${importName}(() => ({ ${scopes.join(", ")} }))`);
...
function loadSetupComponent(virtualId, ctx, root) {
	...
	const rootVars = Object.keys(attachScopes(program, "scope").declarations);
	s.prepend(`const { ${scopes.filter((name) => !rootVars.includes(name)).join(", ")} } = ${HELPER_PREFIX}ctx();\n`);
...
async function hotUpdateSetupComponent({ file, modules, read }, ctx) {
	const getSubModule = (module$1) => {
		const importedModules = Array.from(module$1.importedModules);
		if (importedModules.length === 0) return [];
		return importedModules.filter(({ id }) => id && isSubModule(id)).flatMap((module$2) => [module$2, ...getSubModule(module$2)]);
	};
...
function transformPost(code, _id) {
	const s = new MagicStringAST(code);
	const id = normalizePath(_id);
	if (id.endsWith(".vue")) return transformMainEntry();
	else if (id.includes("type=script")) return transformScript();
	function transformMainEntry() {
		...
		walkAST(program, { enter(node, parent) {
			...
			} else if (node.type === "Identifier" && node.name === "_sfc_main" && (parent?.type === "CallExpression" && parent.callee.type === "Identifier" && parent.callee.name === "_export_sfc" && node.name === "_sfc_main" || parent?.type === "ExportDefaultDeclaration")) s.appendLeft(node.end, "(ctx)");
		} });
		return generateTransform(s, id);
	}
	function transformScript() {
		...
		walkAST(program, { enter(node) {
			if (node.type === "ExportDefaultDeclaration" && node.declaration) {
				const exportDefault = node.declaration;
				s.prependLeft(exportDefault.leadingComments?.[0].start ?? exportDefault.start, `(${HELPER_PREFIX}ctx) => `);
			}
		} });
		return generateTransform(s, id);
	}
